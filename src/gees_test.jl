...
function test()
    ...
    for icase=1:20
        ...
        if norm(B'*B - eye(n)) >= (1<<3) * eps(Float64)
            @show v
            @show beta
            @show B
        end
        @test norm(B'*B - eye(n)) < (1<<3) * eps(Float64)
    ...
    A = X * diagm(D0) / X
    D1,V = eig(A); D1 = sort(D1,rev=true);
    @test norm(D0 - D1) < 1e-11
    gehrd!(A)
    @test norm(tril(A,-2)) < 1e-14
    D2,V = eig(A); D2 = sort(D2,rev=true);
    ...
    gehrd!(A)
    D1,V = eig(A); D1 = sort(D1,rev=true);
    @test norm(D0 - D1) < (1<<4) * eps(Float64)
